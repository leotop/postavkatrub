	...
	<span class="news-item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
		...
		<?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
			<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
				<div class="zagi"><a style="text-decoration: none;" href="<?echo $arItem["DETAIL_PAGE_URL"]?>"><?echo $arItem["NAME"]?></a></div>
			<?else:?>
				<div class="zagi"><?echo $arItem["NAME"]?></div>
			<?endif;?>
		<?endif;?>
		<?if($arParams["DISPLAY_PREVIEW_TEXT"]!="N" && $arItem["PREVIEW_TEXT"]):?>
			<p class="MsoNormal"><?echo $arItem["PREVIEW_TEXT"];?>
                <a href="<?echo $arItem["DETAIL_PAGE_URL"]?>"><?=GetMessage("DETAIL_PAGE_HREF")?></a>            
            </p>
		<?endif;?>
