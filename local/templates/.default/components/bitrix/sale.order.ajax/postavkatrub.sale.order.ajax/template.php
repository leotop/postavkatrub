<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
    ...
    if(empty($_REQUEST["is_ajax_post"])){
        $UserID = $arResult["ORDER_DATA"]["USER_ID"];
        $rsUser = CUser::GetByID($UserID);
        $arUser = $rsUser->Fetch();
        $arChangeFields = array(
            23 => $arUser["NAME"]." ".$arUser["LAST_NAME"],
            24 => $arUser["PERSONAL_PHONE"],
            25 => $arUser["EMAIL"],
            27 => $arUser["NAME"]." ".$arUser["LAST_NAME"],
            28 => $arUser["WORK_PHONE"],
            29 => $arUser["EMAIL"],            
            31 => $arUser["WORK_COMPANY"], //Название компании         
            35 => $arUser["WORK_POSITION"], //Должность          
        ); 
           
        foreach($arResult["ORDER_PROP"]["USER_PROPS_Y"] as $UserPropsID => $arUserProps) {
            foreach($arChangeFields as $PropsID => $PropsValue) {
                if($UserPropsID == $PropsID){                                
                    $arResult["ORDER_PROP"]["USER_PROPS_Y"][$UserPropsID]["VALUE"] = $PropsValue;
                    $arResult["ORDER_PROP"]["USER_PROPS_Y"][$UserPropsID]["~VALUE"] = $PropsValue;     
                }    
            }    
        }  
