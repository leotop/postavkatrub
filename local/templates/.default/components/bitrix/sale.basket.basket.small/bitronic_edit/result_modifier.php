<?if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) {
    ...
    foreach($arResult['ITEMS'] as $ItemID => $arElement) {
        foreach($arResult['MEASURE'] as $MeasureProp) {
            if($arResult['ITEMS'][$ItemID]['PRODUCT_ID'] == $MeasureProp["ID"]) {
                $arResult['ITEMS'][$ItemID]['MEASURE'] = $MeasureProp["PROPERTY_MEASURE_VALUE"];        
            }     
        }      
    }
    // functions
        ...
        foreach ($arResult['ITEMS'] as &$arItem) {
            if ($arItem['CAN_BUY'] == 'Y') {
                ...
                if (CModule::IncludeModule("iblock") /*  && $arParams['IMAGE'] */) {
                    ...
                    $cache_id = 'yen-bs-fly1026' . $arParams['VIEW_PROPERTIES'] . $arItem['PRODUCT_ID'];
    ...
    // js:
    $arResult['TEMPLATE_PATH'] = $pathToTemplateFolder; 
    ...
    $(function(){
        ...
        $(window).scroll(function () {
            if ($(window).scrollTop() >= <?=$arParams['START_FLY_PX'];?>) {
                ...
                $('.yen-bs-box .yen-bs-count').css('left', 0-parseInt($('.yen-bs-count').css('right'))+'px');
                <?if($bitronic):?>
